                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-5">
                                <div class="article-thumb">
                                    <a href="{{ route('user.car.details',[$data->carId]) }}"> <img src="{{ asset($data->image) }}" class="img-responsive car-pic" alt="Image" /></a>
                                </div>
                            </div>
                                    ...
                                    <div class="display-table-cell">
                                        <div class="article-body">
                                            <div class="article-date">
                                                ...
                                                @elseif( $currentTimeDriverLeave >0  || $currentTimeCarMaintance >0 || $currentTimeCarRequisition >0  )
                                            ...
                                            <div class="text-center">
                                                <table class="table">
                                                    ...
                                                    @if ($drverLeave)
                                                        <span class="bg-warning text-capitalize pl-1 pr-1 h6">Driver Leave : {{ date("M j, Y", strtotime($drverLeave->start))  }} <b class="text-dark">To</b> {{ date("M j, Y", strtotime($drverLeave->end))  }}</span>

                                                    {{-- Car Maintence Status Check--}}
                                                    @elseif($carMaintance)
                                                       <span class="bg-info text-capitalize text-white pl-1 pr-1 h6">Maintenance : {{ date("M j, Y", strtotime($carMaintance->start))  }} <b class="text-dark">To</b> {{ date("M j, Y", strtotime($carMaintance->end)) }}</span>

                                                    {{-- Car Requisition Status Check--}}
                                                    @elseif($carRequisition)
                                                       <span class="bg-primary text-capitalize text-white pl-1 pr-1 h6">Requisition : {{ date("M j, Y", strtotime($carRequisition->start)) }} <b class="text-dark">To</b> {{ date("M j, Y", strtotime($carRequisition->end)) }}</span>
                                                    @endif
                                                ...
                                                <a href="{{ route('regular.car.booking',[$data->carId]) }}" class="readmore-btn">Book <i class="far fa-arrow-alt-circle-right"></i></a>
                            ...
                            <div class="col-lg-2">
                                <div class="article-thumb-s">
                                    <a href="{{ route('driver.profile',$data->driverId) }}"> <img src="{{ asset($data->driverImage) }}" class="img-responsive mx-auto d-block" alt="Image" /> </a>
                                    <p>{{ $data->driverName }}</p>
                                    <p><i class="fas fa-mobile-alt"></i> <a href="tel:+88{{ $data->contact }}"> {{ $data->contact }} </a></p>
                                </div>
                            </div>
                            <!--  Driver Section End -->
